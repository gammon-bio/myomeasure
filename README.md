...
[![Tests](https://github.com/gammon-bio/myomeasure/actions/workflows/tests.yml/badge.svg)](https://github.com/gammon-bio/myomeasure/actions/workflows/tests.yml)
...
`tests/test_phantom.py` builds synthetic "phantom" myotubes of known width
in memory -- straight tubes, curved tubes, and objects designed to be rejected
by the shape filters -- and checks the measured diameters against ground truth.
Nothing is read from disk, and Cellpose and PyTorch are not imported, so the
suite runs in seconds against a plain scientific Python stack.

```bash
pytest tests/ -q
```

This is also the container build gate: the Dockerfile runs `pytest tests/ -q`,
so a non-zero exit fails the image build. A published image is therefore one
where these tests passed. The same suite runs in CI on every push and pull
request (see the badge above).
